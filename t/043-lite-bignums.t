...
subtest 'SCALARS' => sub {
    for my $ip_str (sort keys %num) {
        my $n  = $num{$ip_str};
        my $ip = NetAddr::IP::Lite->new($n);
        is($ip, $ip_str, "$n\t=> got: $ip\texp: " . $ip_str);
    }
};

subtest 'Math::BigInt' => sub {
    for my $ip_str (sort keys %num) {
        my $n  = Math::BigInt->new($num{$ip_str});
        my $ip = NetAddr::IP::Lite->new($num{$ip_str});
        is($ip, $ip_str, "$n\t=> got: $ip\texp: " . $ip_str);
    }
};
